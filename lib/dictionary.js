...
function readLocation(fileStream, oldLibrary, newLibrary) {
  // This may be recursive, we might need to pass a library in here
  var streamId = readUVInt(fileStream);
  var definitionId = streamIdToDefinitionId(oldLibrary, streamId);
  var reader = getReader (oldLibrary,definitionId);
  var location = reader(fileStream,oldLibrary,newLibrary);
  return location;
}

function readDefinition (fileStream, oldLibrary, newLibrary) {
  oldLibrary = oldLibrary || newLibrary;
  var id = readUVInt(fileStream);
  var location = readLocation(fileStream,oldLibrary,newLibrary);
...
function readDefinitions(fileStream,oldLibrary,newLibrary) {
  ...
  newLibrary = libfns.buildStructures(oldLibrary, newLibrary);
  return newLibrary;
}
...
var readCore = (function() {
  ...
  var readCoreBody = function(fileStream) {
    ...
    var newLibrary = libfns.makeBaseLibrary(1.3);
    verify(bodySize, newLibrary);
    return readDefinitions(fileStream,null,newLibrary);
  };
  ...
  return function(fileStreamAndLibrary) {
    ...
    for(var i = 0; i < noExtensions;  i++) {
      ...
      newLibrary = readDefinitions (fileStream, null, newLibrary);
    }
