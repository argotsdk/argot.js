...
function getReader(library, definitionId, streamIdToNameMap) {
  ...
  if (typeName) {
    logger.debug('Looking up reader for type: %s', typeName);
    var typeDef = libfns.getTypeDefinition(library,'core',typeName);
...
function readDefinition (fileStream, library, streamIdToNameMap, core) {
  ...
  return libfns.registerType(library,core,streamIdToNameMap,id,location,structure);
}

function readDefinitions(fileStream,library,core) {

  var streamIdToNameMap = libfns.makeBaseStreamIdToNameMap(library,'core');
  var numberOfDefinitions = readUVInt (fileStream);
  for(var i = 0; i < numberOfDefinitions;  i++) {
    var libraryAndStreamIdMap = readDefinition(fileStream, library, streamIdToNameMap, core);
...
var readMessage = (function() {
  return function(fileStreamAndLibrary) {
    ...
    for(var i = 0; i < noSections;  i++) {
      ...
      library = readDefinitions(fileStream, library,false);
    }
    return [fileStream, library];
  };
})();
