...
function readDictionaryDefinitionEnvelope(fileStream,refLibrary) {
  var typeDef = libfns.getTypeDefinition(refLibrary,'dictionary.definition_envelope');
  var reader = libfns.getReaderFn(typeDef);
...
function loadDictionary(fileName, cb) {
  var library = libfns.makeBaseLibrary(1.3);
