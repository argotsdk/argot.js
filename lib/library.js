...
function buildStructures2 (oldLibrary,newLibrary,types) {
  ...
  keys.forEach (function (name){
    ...
    if (type.structure) {
      ...
      var readValue = reader (fileStream,oldLibrary,newLibrary);
      logger.info ('Read %s, got %j', name, readValue, {});
      // Now for types that don't have them, we need to build readers and
      // writers using the type expressions.
    }
  });
  return newLibrary;
}
