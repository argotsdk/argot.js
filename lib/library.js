...
function addStreamIdToNameMapping(library,streamId,name) {
  var fullName = getNameFromStreamId(library,streamId);
  if (fullName && fullName !== name) {
    logger.warn('Id %d is already mapped to %s, overriding with %s',
                streamId, getNameFromStreamId(library,streamId), name);
  }
...
function addDefinitionToLibrary(library,definition,streamId) {
  ...
  logger.debug('Added %s to library', getFullName(library, definition));
  return library;
}
...
function makeBaseLibrary(version) {
  ...
  var addBaseDef = function(lib,id,name) {
    var definition = {name:name, parent:'dictionary', reader:readers.metaMappings['dictionary.' + name ]};
    return addDefinitionToLibrary(library, definition, id);
  };
  library = addBaseDef(library,30,'base');
  library = addBaseDef(library,31,'name');
  library = addBaseDef(library,32,'definition');
  library = addBaseDef(library,35,'definition_envelope');
  return library;
}
...
function addReader(library,type) {
  if(!type.reader) {
    var reader = buildReader (library,type);
    if (reader) {
      type.reader = reader;
    }
  }
...
function buildStructures2 (refLibrary,library,types) {
...
function buildStructures(refLibrary,library) {
  ...
  var nameTypes = getTypes (library,isName);
  var definitionTypes = getTypes (library,isDefinition);
  var relationTypes = getTypes (library,isRelation);
  logger.info ('Building %d types {%d names, %d definitions %d relations}',
               Object.keys(library.types).length,
               Object.keys(nameTypes).length,
               Object.keys(definitionTypes).length,
               Object.keys(relationTypes).length);
  library = buildStructures2 (refLibrary,library,nameTypes);
  library = buildStructures2 (refLibrary,library,definitionTypes);
  library = buildStructures2 (refLibrary,library,relationTypes);
  // Should be built in order: names then definitions and then relations
  return library;
}

function read(library,type,data) {
  var typeDef = getTypeDefinition(library,type);
  var reader = getReaderFn(typeDef);
  return reader(data);
}
