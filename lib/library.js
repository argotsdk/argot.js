...
function buildReader(library,type) {
  var makeAugmentor = function(description) {
    return function(fileStream, subject) {
      var value = reader_fn(fileStream);
  ...
  if (type.expression) {
    ...
    for(var i = 0; i < type.expression.length;  i++) {
      ...
      if (segment.description && segment.expression) {
        ...
        var expression_type = segment.expression.type;
        var reader_fn = getReaderFn(getTypeDefinition(library,expression_type));
        var augmentor = makeAugmentor(description);
...
function buildStructures2 (oldLibrary,newLibrary,types) {
  ...
  keys.forEach (function (name){
    ...
    if (type.structure) {
      ...
      type.type_reader = buildReader (newLibrary,type);
    }
  });
  return newLibrary;
}
