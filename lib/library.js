...
function getRegisterFunction (library, id, location) {
  if (location && location.group != null  && location.name && location.version) {
    return registerDefinition;
  } else if (location && location.group != null && location.name) {
    return registerName;
  } else  if (location && location.id != null && location.tag) {
...
function registerType(library, id, location, structure) {
  // TODO update to handle names, definitions and relations
  // instead of treating everything like a definition
  // And record the type so that we can fall back on it later when
  // we need to read the structures
