...
function readMetaDefinition(fileStream,library) {
  ...
  if (reader) {
    return reader(fileStream,library);
  } else {
...
function readMetaSequence(fileStream,library) {
  var sequenceSize = readUInt8(fileStream);
  var sequence = [];
  for(var i = 0; i < sequenceSize;  i++) {
    var readType = readMetaDefinition(fileStream,library);
...
function readMetaReference(fileStream) {
  var type = readUVInt(fileStream); // what type is this? A stream id?
  return {type:type};
}

function readMetaTag(fileStream,library) {
  var description = readUtf8(fileStream);
  var expression = readMetaDefinition(fileStream,library);
  return {description:description, expression:expression};
}

function readMetaArray(fileStream,library) {
  var sizeType = readMetaDefinition(fileStream,library);
  var typeType = readMetaDefinition(fileStream,library);
...
var readMetaAbstract = readMetaDefinition;

function readMetaEncoding(fileStream,library) {
  var expression = readMetaDefinition(fileStream,library);
